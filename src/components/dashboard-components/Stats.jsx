...
const Stats = () => {
  ...
  const { totalRevenueAllTime, todayRevenue, yesterdayRevenue, totalUsersCount } = useSelector(
    (state) => state.admin,
  );
  ...
  useEffect(() => {
    let change =
      yesterdayRevenue === 0&&todayRevenue === 0 ? 0:yesterdayRevenue === 0 ? 100 : ((todayRevenue - yesterdayRevenue) / yesterdayRevenue) * 100;
    const revenueChangeText = `${change >= 0 ? '+' : '-'}${change.toFixed(2)}% from yesterday`;
    setRevenueChange(revenueChangeText);
  }, [yesterdayRevenue]);
